...
FW_AKDE <- AKDE_wild[c("Bumpus", "Cate", "Elaine", "Makao", "Puji", "Annie", "Hannah", "Jane",
                       "Margaret", "Maria", "Sheron")]
FW_FITS <- FITS_wild[c("Bumpus", "Cate", "Elaine", "Makao", "Puji", "Annie", "Hannah", "Jane",
                       "Margaret", "Maria", "Sheron")]
FW_SPEED <- SPEED_wild[c("Bumpus", "Cate", "Elaine", "Makao", "Puji", "Annie", "Hannah", "Jane",
                         "Margaret", "Maria", "Sheron")]

MW_AKDE <- AKDE_wild[c("Anthony", "Christoffer", "Jackson", "Kyle", "Little_Rick", "Alexander",
                       "Beto", "Larry", "Luigi", "Reid", "Rodolfo", "Thomas")]
MW_FITS <- FITS_wild[c("Anthony", "Christoffer", "Jackson", "Kyle", "Little_Rick", "Alexander",
                       "Beto", "Larry", "Luigi", "Reid", "Rodolfo", "Thomas")]
MW_SPEED <- SPEED_wild[c("Anthony", "Christoffer", "Jackson", "Kyle", "Little_Rick", "Alexander",
                         "Beto", "Larry", "Luigi", "Reid", "Rodolfo", "Thomas")]

# get averages for each sex
# female wild-raised populations
# speed
SPEED <- ctmm::meta(FW_SPEED, variable = "speed", level = 0.95, units = FALSE)
...
# home range size 
HR <- ctmm::meta(FW_AKDE, variable = "area", level = 0.95, units = FALSE)
HR <- as.data.frame(HR)
...
# tau velocity
TAUVELOCITY <- ctmm::meta(FW_FITS, variable = "tau velocity", level = 0.95, units= FALSE)
...
# combine all
META_total_FW <- cbind(SPEED, TAUVELOCITY, HR)
# add information about group
META_total_FW$Sex <- "Female"
META_total_FW$Status <- "Wild-raised"


# male wild-raised populations
# speed
SPEED <- ctmm::meta(MW_SPEED, variable = "speed", level = 0.95, units = FALSE)
SPEED <- as.data.frame(SPEED)
SPEED <- SPEED[-c(2,3),] # removes unnecessary rows
SPEED[,"low"] <- "km/day" %#% SPEED[,"low"] # converts m/s to km/day 
SPEED[,"est"] <- "km/day" %#% SPEED[, "est"] # converts m/s to km/day 
...
# home range size 
HR <- ctmm::meta(MW_AKDE, variable = "area", level = 0.95, units = FALSE)
HR <- as.data.frame(HR)
HR <- HR[-c(2,3),] # removes unnecessary rows
HR[,"low"] <- "km^2" %#% HR[,"low"] # converts m^2 to km^2
HR[,"est"] <- "km^2" %#% HR[, "est"] # converts m^2 to km^2
...
colnames(HR) <- c("HR_low", "HR_est", "HR_high")
# tau velocity
TAUVELOCITY <- ctmm::meta(MW_FITS, variable = "tau velocity", level = 0.95, units= FALSE)
TAUVELOCITY <- as.data.frame(TAUVELOCITY)
TAUVELOCITY <- TAUVELOCITY[-c(2,3),] # removes unnecessary rows
TAUVELOCITY[,"low"] <- "minutes" %#% TAUVELOCITY[,"low"] # converts seconds to minutes
TAUVELOCITY[,"est"] <- "minutes" %#% TAUVELOCITY[, "est"] # converts seconds to minutes
...
colnames(TAUVELOCITY) <- c("τvelocity_low", "τvelocity_est", "τvelocity_high")
# combine all
META_total_MW <- cbind(SPEED, TAUVELOCITY, HR)
# add information about group
META_total_MW$Sex <- "Male"
META_total_MW$Status <- "Wild-raised"


# bind them all
META_df <- rbind(META_total_FW, META_total_MW)
...
# save final plot
ggsave("./FIGURES/Revised/Supplementary_Figure_4.png", width = 8.5, height = 5.5, units = "in", 
